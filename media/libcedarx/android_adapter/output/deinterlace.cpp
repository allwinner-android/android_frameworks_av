...
static int __DiSetMode2(Deinterlace* di )
{
                    ...
                    int nField)
{
    ...
    {
        int nFrameRate = 30000; //* we set the frameRate to 30
        pOutPicture->nPts = pNextPicture->nPts + nField * (1000 * 1000 * 1000 / nFrameRate) / 2;
    }
    else
    {
        if((pNextPicture->nPts - pCurPicture->nPts) > 0)
        {
            pOutPicture->nPts = pNextPicture->nPts + nField * (pNextPicture->nPts - pCurPicture->nPts)/2;
        }
        else
        {
            pOutPicture->nPts = pNextPicture->nPts + 16*1000;
            logv("pNextPicture->nPts = %lld ms, pCurPicture->nPts = %lld ms, pPrePicture->nPts = %lld ms,\n \
                 pOutPicture->nPts = %lld ms, diff = %lld ms ",
                 pNextPicture->nPts/1000, pCurPicture->nPts/1000,
                 pPrePicture->nPts/1000, pOutPicture->nPts/1000,
