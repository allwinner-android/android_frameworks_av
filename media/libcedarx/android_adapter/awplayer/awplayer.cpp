...
static void setupPlayerStatus(int status);
...
static void setupPlayerStatus(int status) {
    char value[PROP_VALUE_MAX] = {0};
  	GetCallingApkName(value, sizeof(value));
    property_set("zidoo.playing.apk", value);
    sprintf(value, "%d", status);
    property_set("zidoo.playing.status", value);
