...
static int SbmFrameAvcInit(SbmInterface* pSelf, SbmConfig* pSbmConfig)
{
    ...
    if(pSbmBuf == NULL)
    {
        loge(" palloc for sbmBuf failed, size = %d MB", pSbm->mConfig.nSbmBufferTotalSize/1024/1024);
        goto ERROR;
    }
    ...
    for(i = 0; i < MAX_FRAME_PIC_NUM; i++)
    {
        ...
        memset(pSbm->mFramePicFifo.pFramePics[i].pNaluInfoList, 0, DEFAULT_NALU_NUM*sizeof(NaluInfo));
...
static int SbmFrameAvcReturnFramePic(SbmInterface* pSelf, VideoStreamDataInfo *pDataInfo)//* returnFramePic
{
    SbmFrameAvc* pSbm = (SbmFrameAvc*)pSelf;
...
static int SbmFrameAvcFlushFramePic(SbmInterface* pSelf, VideoStreamDataInfo *pDataInfo) //* flushFramePic
{
    SbmFrameAvc* pSbm = (SbmFrameAvc*)pSelf;
...
**********************************************************************
*/
static int flushStream(SbmFrameAvc *pSbm, VideoStreamDataInfo *pDataInfo)
{
    int nFlushPos;
    ...
    logv("flush stream, pDataInfo = %p, pos = %d, %p",pDataInfo, nFlushPos,&pSbm->frameFifo.pFrames[nFlushPos]);
...
static inline int supplyStreamData(SbmFrameAvc* pSbm)
{
    ...
    {
        flushStream(pSbm, pDetectInfo->pCurStream);
...
static inline void storeNaluInfo(SbmFrameAvc* pSbm, int nNaluType, int nNalRefIdc, int nNaluSize, char* pNaluBuf)
{
    DetectFramePicInfo* pDetectInfo = &pSbm->mDetectInfo;
...
static void detectWithStartCode(SbmFrameAvc* pSbm)
{
    ...
    while(1)
    {
        ...
        logv("gqy***nNaluType =%d, store nalu size = %d, nNalRefIdc = %d", nNaluType, nNaluSize, nNalRefIdc);
        //*6. store  nalu info
        storeNaluInfo(pSbm, nNaluType, nNalRefIdc, nNaluSize, pDetectInfo->pCurStreamDataptr);
